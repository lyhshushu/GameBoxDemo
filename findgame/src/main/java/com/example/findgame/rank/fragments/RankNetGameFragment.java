...
public class RankNetGameFragment extends BaseFragment {
    ...
    private List<AllRankBean> rankNetGameBeans;
    private Context mContext;
    private Handler handler;
    private RankNewGameAdapter rankNetGameAdapter;
    ...
    protected void initView() {
        ...
        rankNetGameAdapter = new RankNewGameAdapter(R.layout.item_rank_new_game);
        ...
        rvRankNewGame.setAdapter(rankNetGameAdapter);
    }

    @Override
    protected void bindListener() {
        rankNetGameAdapter.setOnItemChildClickListener((adapter, view, position) -> {
            int id = view.getId();
            if (id == R.id.cl_rank_new_game) {
                Toast.makeText(mContext, rankNetGameBeans.get(position).getGameName(), Toast.LENGTH_SHORT).show();
            }
            if (id == R.id.bt_rank_new_game_download) {
                Toast.makeText(mContext, rankNetGameBeans.get(position).getGameName() + getString(R.string.download), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    protected void applyData() {
        rankNetGameBeans = new LinkedList<>();
        rankNetGameBeans = new LinkedList<>();
        getJson(BASEURL + "//app/android/v4.2.2/game-top-startKey--type-net-n-20.html");
        handler = new Handler() {
            @Override
            public void handleMessage(@NonNull Message msg) {
                super.handleMessage(msg);
                if (msg.what == 1) {
                    rankNetGameAdapter.setNewData(rankNetGameBeans);
                    rankNetGameAdapter.notifyDataSetChanged();
                }
            }
        };
    }
    ...
    private void getRankNetGame(String json) {
        try {
            ...
            for (int i = 0; i < dataJson.length(); i++) {
                ...
                rankNetGameBeans.add(newGameBean);
            }
